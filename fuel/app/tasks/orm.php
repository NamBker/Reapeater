	...
	public function generate_model($table_name, $db = null)
	{
		...
		foreach ($columns as $column)
		{
			...
			elseif ($column['type'] == 'int')
			{
				...
				array_push($column_validation, 'is_numeric');
				//\Cli::error(substr($column['name'], -3, 3));
				$column_validation['numeric_between'] = array(substr($column['name'], -2, 2) == 'id' ? 1 : 0, (int) min($column['max'], $display_max));
			...
			if (in_array($column['name'], array('id')))
			{
				$column_form['type'] = false;
				unset($column_properties);
			}
			...
			else
			{
				...
				switch ($column_type)
				{
					...
					case 'bigint':
						...
						break;
					case 'datetime':
                        $column_properties['data_type'] = 'datetime';
					    $column_properties['expose_type'] = 'null';
                        unset($column_properties['null']);
					/* @TODO
					case 'date':
					case 'datetime':
					case 'time':
            ...
            if($column_type == 'varchar' && substr($column['name'], -2, 2) == 'id') {
                unset($column_properties['validation']['max_length']);
                $column_properties['validation'][] = 'is_numeric';
                $column_properties['validation']['numeric_between'] = array(1, 2147483647);
            }
		...
		if($num_key != 0) {
			$primary_key_str = "protected static \$_primary_key = array(".$primary_key_str.");";
		}
		$model_str = <<<MODEL
<?php

class Model_{$table_class} extends \Model_Standard_Model
{
    {$primary_key_str}
...
MODEL;
        ...
        $model_str = str_replace("0 => ", "", $model_str);
        $model_str = str_replace("1 => ", "", $model_str);
